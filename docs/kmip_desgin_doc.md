...
Your Code
    │
    │  "Encrypt this DEK using KEK 249"
    ▼
┌─────────────────────────────────────────────┐
│  KMIP Context (KMIP*)                        │
│  Encodes your request into TTLV binary bytes │
│  Decodes the response back into structs      │
└──────────────────┬──────────────────────────┘
                   │ raw bytes
                   ▼
┌─────────────────────────────────────────────┐
│  BIO (BIO*)                                  │
│  OpenSSL's I/O wrapper                       │
│  You write bytes to it, it handles the rest  │
│  It connects SSL + socket together           │
│                                              │
│  ┌───────────────────────────────────────┐   │
│  │  SSL (SSL*)                           │   │
│  │  Encrypts/decrypts the data           │   │
│  │  Handles TLS handshake                │   │
│  │  Uses SSL_CTX for certificates        │   │
│  │                                       │   │
│  │  ┌───────────────────────────────┐    │   │
│  │  │  TCP Socket (fd)              │    │   │
│  │  │  The actual network connection│    │   │
│  │  │  to KMIP server port 5696    │    │   │
│  │  └───────────────────────────────┘    │   │
│  └───────────────────────────────────────┘   │
└──────────────────┬──────────────────────────┘
                   │ encrypted bytes over TCP
                   ▼
            KMIP Server
...
S3 request 1 ─┐
S3 request 2 ─┤
S3 request 3 ─┼─► queue ──► [worker 0] ──► TLS conn ──► KMIP server
S3 request 4 ─┤              processes
S3 request 5 ─┘              one at a time
                              (100ms each = 500ms total)
Multiple workers (after):


S3 request 1 ─┐              [worker 0] ──► TLS conn 0 ──► KMIP server
S3 request 2 ─┤              [worker 1] ──► TLS conn 1 ──► KMIP server
S3 request 3 ─┼─► queue ──► [worker 2] ──► TLS conn 2 ──► KMIP server
S3 request 4 ─┤              [worker 3] ──► TLS conn 3 ──► KMIP server
S3 request 5 ─┘              processes
                              four at a time
                              (100ms each, 4 parallel = 125ms total)